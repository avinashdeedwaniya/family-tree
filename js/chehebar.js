...
// memebr info simply slides in and don't push the tree left, but only zoom buttons
...
$(document).ready(function () {
	...
	hexPinkImg
	    ...
	    .attr("src", plugin_url.pluginurl+"img/hex-pink.png");	
	
	var hexBluImg = new Image();
	hexBluImg.src = plugin_url.pluginurl+"img/hex-blu.png";
	...
	$.getJSON(plugin_url.json_file_url,function(json){		
	//$.getJSON("chehebar-tree3.json",function(json){		
		var oTree = json;
		DisplayTree (oTree,0);	
	});
	...
	function DisplayNameOrigin(){
		...
		var sNameOrigin =   $("#hidden_data").html();
...
function updateMemberInfo($node, updated) {
	...
	$('.member-content').load(plugin_url.pluginurl+'getUserData.php?id=' + contesto.attr('rel-id') + '&top_family_id=' + top_family_id + '&id_family=' + family_id + '&sex=' + sex + '&isSingle=' + isSingle + '&treepassword='+ treePassword + '&adm=' + adm + '&tipo=' + tipo + '&image=' + contesto.attr('rel-photo'), function() {	
		updated();
	});		
}
